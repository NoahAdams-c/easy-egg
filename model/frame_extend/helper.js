 ...
 * @LastEditTime: 2021-07-26 15:12:02
 */
...
module.exports = {
  ...
  isValid(props, requireKeys) {
    ...
    const { hasValueNull, invalidKeys } = this.isValueNull(props, requireKeys)
    const result = {}
    if (isLost) {
  ...
  handleFail(statusCode = 40000, res = {}) {
    const { ctx } = this
    try {
      const { errcode, errmsg, errorCode } = ctx.helper.errcode[statusCode]
  ...
  encrypteToken(token) {
    ...
    return (
      token.slice(0, startIndex) + encryptedTokenPart + token.slice(endIndex)
    )
  },
  ...
  decrypteToken(encrytedToken) {
    ...
    const originTokenPart = this.doDecrypte(encryptedOpts[0], encryptedOpts[1])
    originToken =
   ...
   */
  genServiceDesign() {
    const projectPath = this.app.baseDir
